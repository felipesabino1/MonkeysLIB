...
int main() {
	...
	for (int i = 1; i < c; i++) {
		if (p2[i] != p2[i-1]) {
			// cout << "adding " << p2[i].first << " " << p2[i].second << endl;
			p[n++] = p2[i];
		}
	}
	...
	// 	cout << lower[i].first << ", " << lower[i].second << endl;
	// }
 
	// cout << "upper: \n";
	// for (int i = 0; i < uppersize; i++) {
	// 	cout << upper[i].first << ", " << upper[i].second << endl;
	// }
	...
	while (i < uppersize-1 || j > 0) {
		// cout << "(" << i << ") " << upper[i].first << ", " << upper[i].second << " dist to " 
		// 	<< "(" << j << ") " << lower[j].first << ", " << lower[j].second
		// 	<< " = " << dist(upper[i], lower[j]) << endl;
